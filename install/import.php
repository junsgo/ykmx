...
if($_GET['action'] == 'import'){
	...
	$sql_arr[] = "INSERT INTO `".$prefix."admin` (uid,mid,username,userpass) VALUES ('1','0','$admin_user','$admin_pass')";
...
function open_sql($prefix){
	...
	$sql = str_replace('DROP TABLE IF EXISTS `',"DROP TABLE IF EXISTS `".$prefix,$sql);
	$sql = str_replace('CREATE TABLE `',"CREATE TABLE `".$prefix,$sql);
	$sql = str_replace('INSERT INTO `',"INSERT INTO `".$prefix,$sql);
	$sql = str_replace('IF EXISTS `',"IF EXISTS `".$prefix,$sql);
