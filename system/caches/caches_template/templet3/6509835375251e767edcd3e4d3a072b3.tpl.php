		...
		<ul>        	
			...
			<li class="autolist autolistEnd jinrishop">         
				...
				<div class="listEndInfo">
					...
					<p>恭喜<a href="<?php echo WEB_PATH; ?>/uname/<?php echo idjia($shop['q_uid']); ?>" target="_blank"><?php echo get_user_name($shop['q_uid']); ?></a>获得<br>幸运梦想码：<span class="orange"><?php echo $shop['q_user_code']; ?></span><br>揭晓时间：<span>
					...
					<ul class="autoOldlist">
						...
						<li class="uInfo">
							...
							<dt class="uget">恭喜<a href="<?php echo WEB_PATH; ?>/uname/<?php echo idjia($eshop['q_uid']); ?>" target="_blank"><?php echo get_user_name($eshop['q_uid']); ?></a>获得<br>
								幸运梦想码：<span class="orange"><?php echo $eshop['q_user_code']; ?></span><br>
	...
	<div class="autoRule">
		...
		<div class="autoOldC">
			<ul>
				...
				<li>2、如果计算出的梦想码未被购买，则取差值最小的梦想码作为幸运梦想码。</li>
				<li>3、如果有2个梦想购码最小差值相等，则取大值作为最终幸运梦想码。</li>
				<li>4、限时揭晓商品不参与差价送福分活动且晒单不可再获得1000福分奖励。</li>
				<li>5、限时揭晓的幸运梦想码计算方式：</li>
				<li>&nbsp; 1) 限时揭晓商品取截止时间前网站所有商品100条购买时间记录。</li>
				<li>&nbsp; 2) 时间按时、分、秒、毫秒依次排列组成一组数值。</li>
				<li>&nbsp; 3) 将这100组数值之和除以商品总需参与人次后取余数，余数加上10,000,001即为“幸运梦想码”。</li>
			</ul>
		</div>
	</div>
</div>
