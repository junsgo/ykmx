...
class cart extends base {
	...
	public function paysubmit(){
		...
		_session_start();	
		 ...
		 $checkpay=_htmtocode($this->segment(4));//获取支付方式  fufen   money  bank
		 $banktype=_htmtocode($this->segment(5));//获取选择的银行 CMBCHINA  ICBC CCB
		 $money=_htmtocode($this->segment(6));   //获取需支付金额
		 $fufen=_htmtocode($this->segment(7));  	 //获取福分
		 $submitcode1=_htmtocode($this->segment(8));   //获取SESSION
		 ...
		 if($checkpay != "money" && $checkpay != "fufen" && !empty($checkpay)){
			$checkpay = safe_replace($checkpay);
	...
	public function addmoney(){
		...
		if($checkpay != "money" && $checkpay != "fufen" && !empty($checkpay)){
         ...
         // $zhifutype = $this->db->GetOne("select * from `@#_pay` where `pay_class` = '$pay_class' and `pay_mobile` = '1'"); 
