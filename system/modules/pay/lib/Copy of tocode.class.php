...
// 		$this->get_code_user_html();
	...
	private function get_code_dabai(){
		...
		$ys = fmod($count_time,$this->cyrs);
		
		
		if($white){
			$ysary = array();
			$goucodes = explode(",", $white['goucode']);
			foreach ($goucodes as $code){
				$temp = bcsub($code, 10000001);
				$ysary[] = bcsub($code, 10000001);				
			}
			if(! in_array($ys,$ysary)){
	...
	private function get_code_user_html(){		
		...
		$this->white_list = $this->db->GetList("select uid from `@#_member_white` where `shopid` = '$this->shopid");
